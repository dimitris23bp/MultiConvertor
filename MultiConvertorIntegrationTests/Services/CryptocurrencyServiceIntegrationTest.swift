...
final class CloudKitTests: XCTestCase {
    ...
    func testFetchAllRecords() async throws {
        // We test the CloudKitService directly for integration
        let service = await CloudKitService()
        
        // Use an expectation to handle the async nature of the call
        do {
            let cryptos = try await service.fetchCryptocurrenciesFromCK(amount: 50)
            ...
            for crypto in cryptos.prefix(5) { // Print first 5 for sanity check
                print("Record ID: \(await crypto.name)")
            }
        } catch {
